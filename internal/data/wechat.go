...
func (r *wechatRepo) GetOAUser(ctx context.Context, unionID string) (m *biz.WechatOAUser, err error) {
	err = r.data.db.WithContext(ctx).First(&m, "unionid=?", unionID).Error
	if err != nil {
		return nil, errors2.WithStack(err)
	}
	return
...
func (r *wechatRepo) GetOAUserByOpenid(ctx context.Context, openidList []string) (m []*biz.WechatOAUser, err error) {
	err = r.data.db.WithContext(ctx).Find(&m, "openid in ?", openidList).Error
	if err != nil {
		return nil, errors2.WithStack(err)
	}
	return
...
func (r *wechatRepo) UpsertOAUser(ctx context.Context, m []*biz.WechatOAUser) error {
	err := r.data.db.WithContext(ctx).Clauses(clause.OnConflict{
		Columns:   []clause.Column{{Name: "unionid"}, {Name: "unionid"}},
