...
func TwoByBitmap(img image.Image) *WorldMap {
	worldMap := &WorldMap{
...
func GetTextBound(this *WordCloudRender, fontSize float64, text string) (w, h, xdiff, ydiff float64) {

	measureDc := this.MeasureDc
	...
	if measureDc.Width() < x {
		x = measureDc.Width()
	}
	if measureDc.Height() < y {
		this.UpdateFontSize(20)
		y = measureDc.Height()
	}

	measureDc.DrawStringAnchored(text, float64(x), float64(y), 0.5, 0.5)
	img := measureDc.Image()
